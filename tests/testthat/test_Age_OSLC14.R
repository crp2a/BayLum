...
test_that("Full function test", {
  ...
  expect_s3_class(Age_OSLC14(DATA=Data,Data_C14Cal=C14Cal,Data_SigmaC14Cal=SigmaC14Cal,
                    SampleNames=c("GDB5",Names,"GDB3"),Nb_sample=3,SampleNature=samplenature,
                    PriorAge=prior,StratiConstraints=SC,Iter=50,n.chains=2), class = "BayLum.list")

  ##crash function for wrong ordering
  samplenature=matrix(data=c(0,1,1,1,0,0),ncol=3,nrow=2,byrow=TRUE)
  expect_error(
    Age_OSLC14(
      DATA=Data,Data_C14Cal=C14Cal,Data_SigmaC14Cal=SigmaC14Cal,
      SampleNames=c("GDB5",Names,"GDB3"),Nb_sample=3,SampleNature=samplenature,
