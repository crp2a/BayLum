...
#'  }
#'  \item\bold{The Gelman and Rudin test of convergency}: prints the result of the Gelman and Rudin test of convergency for palaeodose and equivalent dose dispersion parameters for each sample.
...
#' # P=Palaeodose_Computation(DATA1,Nb_sample=1,SampleNames=c("GDB5"),Taille=10000)
#'
#' @references
...
Palaeodose_Computation<-function(DATA,SampleNames,Nb_sample,
  ...
  jags <- jags.model(textConnection(Model_Palaeodose[[Model_GrowthCurve]][[distribution]]), data = dataList, n.chains = Nb_chaines, n.adapt=Taille)
  update(jags,Taille)
