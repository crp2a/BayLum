...
#' or be more precise on the \code{PriorAge} parameter
#' (for example specify if it is a young sample \code{c(0.01,10)} or an old sample \code{c(10,100)}) to reach convergency.
...
AgeS_Computation<-function(DATA,SampleNames,Nb_sample,
  ...
  jags <-  rjags::jags.model(textConnection(Model_AgeS[[Model_GrowthCurve]][[distribution]]), data = dataList, n.chains = Nb_chaines, n.adapt=Taille)
  update(jags,Taille)
  echantillon =  rjags::coda.samples(jags,c("A","D","sD"),min(Taille,10000),thin=t)
  ...
  R=matrix(data=NA,ncol=8,nrow=3*Nb_sample,
           dimnames=list(rnames,c("lower bound at 95%","lower bound at 68%","Bayes estimate","upper bound at 68%","upper bound at 95%","","Convergencies: Bayes estimate","Convergencies: uppers credible interval")))
  ...
  Info=list("Sampling"=echantillon,"Model_GrowthCurve"=Model_GrowthCurve, "Distribution"=distribution,"PriorAge"=PriorAge,
            "StratiConstraints"=StratiConstraints,"CovarianceMatrix"=THETA)
  return(Info)
