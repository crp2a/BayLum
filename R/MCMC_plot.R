...
#'
#' This function uses the ouput of \code{\link{coda.samples}} to plot the
#' trajectories of MCMC and densities of the posterior distributions
...
#'
#' @param sample [mcmc.list] (**required**): output generated by \code{\link{jags.model}} in \code{\link{Age_Computation}}.
#'
#' @param size [integer] (**required**): length of each chain.
#'
#' @param SampleNames [character] (with default): names of the samples, used in the figure titles.
#'
#' @param Nb_sample [integer] (with default): number of analysed samples.
#'
#' @param Nb_chaines [integer] (with default): number of independent chains for the model
#' (for more informations, see \code{\link{jags.model}}).
#'
#' @param value [integer] vector (with default): position idex used to select age (if available),
#' palaeodose and equivalent dose dispersion parameters for the first sample.
#'
#' @param param [character] vector (with default): names of the selected
#' parameters with value \code{A} for age (if available), \code{D} for palaeodose
#' and \code{sD} for equivalent dose dispersion.
#'
#' @return
