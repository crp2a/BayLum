#' Generates, from one (or several) BIN file(s), a list of luminescence data and information before statistical analysis 
#' 
#' This function is used to generate, from the BIN file(s), a list of values of: 
#' OSL intensities and associated uncertainties, regenerative doses, etc., which will be the input of the Bayesian models.
#' To be easy-to-use, this function requires a rigorous organisation - all needed files should be arranged in one folder - 
#' of informations concerning each BIN file. It is possible to process data for various samples simultaneously and to consider more 
#' than one BIN file per sample. 
#'
#' @param Path character \bold{(required)}: the path to the project folder, containing one or more subfolders in which the BIN files 
#' are located.
#' @param Names character vector \bold{(required)}: list of names of the sub-folders containing the BIN files
...
#' \code{Names} vector.  
#' @param Nb_sample integer \bold{(required)}: number of samples.
...
#' ## give the name of the path to data folder
#' # Path=c("inst/extdata/")
#' ## give the name of the folder datat
#' # Names=c("GDB3")
