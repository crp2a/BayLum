#' @title Write BayLum Files
#'
#' @description This function allows the user to write all .csv files expected by 
#' [Generate_DataFile] and [Generate_DataFile_MG]. Unlike [Create_TemplateFolder],  
...
#' 
#' @author Frederik Baumgarten, RadPhys, DTU Physics, Technical University of Denmark
#'
...
#'
#' @return There will be no output if the function ran as intented.
#'
#' @examples
#' # example samples
...
#' BinPerSample <- c(1,3)
#'
#' # List of data.frames of accepted aliquot/grain to be included in the analysis for each .bin-file.  
#' DiscPos <- list(
...
#' write_BayLumFiles(
#' folder = paste(getwd(),"new_BayLum_folder",sep = "/")
...
#' @md
#' @export
write_BayLumFiles <- function(folder = "BayLum_files",
                               SampleNames,
